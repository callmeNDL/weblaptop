    ...
    try {
      ...
      if (res?.status === 'OK') {
        ...
        setData({
          ...
          MANHINH: thuocTinhs?.filter((item) => item?.tenThuocTinh === 'Màn hình')?.map((el) => el.giaTriThuocTinh) ?? [],
      ...
      <div className="item-left">
        <div className="imageContainer">
          ...
          <div className="product-decs">
            ...
            <br />
            - Màn hình:{data?.MANHINH?.map((item, index) => `${index === (data?.CPU?.length - 1) ? item : (item + ' / ')}`)}
            <br />
            - RAM: {data?.RAM?.map((item, index) => `${index === (data?.CPU?.length - 1) ? item : (item + ' / ')}`)} <br />
            - Màu: {data?.MAU?.map((item, index) => `${index === (data?.CPU?.length - 1) ? item : (item + ' / ')}`)}<br />
            - Lưu trữ: {data?.LUUTRU?.map((item, index) => `${index === (data?.CPU?.length - 1) ? item : (item + ' / ')}`)} <br />
          </div>
        </div>
        <div className="itemInformation">
          <div className="item-name">{data?.tenSanPham}</div>
          <Grid container spacing={2}>
            <Grid item xs={6}>Thương hiệu {data?.nhaSanXuat?.tenNhaSanXuat}</Grid>
            <Grid item xs={1}>|</Grid>
            <Grid item xs={5}>Mã SP {data?.id} </Grid>
          </Grid>
          ...
          <div className="button-box">
            ...
            <button className="button-second" onClick={handeAddCard}>Thêm vào giỏ hàng</button>
          </div>
          <div className="line-bottom"></div>
